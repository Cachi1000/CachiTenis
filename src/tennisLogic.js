...
export const createMatchState = (config) => ({
  ...
  currentGame: {
    player1: 0,
    player2: 0,
    isTiebreak: config.format.startsWith('TIEBREAKS_ONLY'),
    isSuperTiebreak: false
  },
  server: config.player1StartsServing !== false ? 1 : 2,
...
export const addPoint = (currentState, winnerId, reason, isFirstServe) => {
  if (currentState.status === 'COMPLETED') return currentState;
  
  const state = cloneState(currentState);
  const loserId = winnerId === 1 ? 2 : 1;
  const pId = `player${winnerId}`;
  const oppId = `player${loserId}`;
  ...
  if (reason === 'ACE') {
    ...
    state.stats[oppId].doubleFaults += 1;
    state.stats[oppId].secondServesTotal += 1;
    state.firstServeFault = false;
  } else {
    if (reason === 'WINNER') {
      state.stats[pId].winners += 1;
    } else if (reason === 'UNFORCED_ERROR') {
      state.stats[oppId].unforcedErrors += 1;
    } else if (reason === 'FORCED_ERROR') {
      state.stats[oppId].forcedErrors += 1;
    }
  ...
  game[`player${winnerId}`] += 1;

  let gameWon = false;

  if (isSuperTiebreak) {
    const pointsToWin = state.config.format === MATCH_FORMATS.CUSTOM && state.config.customRules?.superTiebreakPoints 
      ? state.config.customRules.superTiebreakPoints : 10;
    if (game[`player${winnerId}`] >= pointsToWin && game[`player${winnerId}`] - game[`player${loserId}`] >= 2) {
      gameWon = true;
    } else {
    ...
    const pointsToWin = state.config.format === MATCH_FORMATS.CUSTOM && state.config.customRules?.tiebreakPoints 
      ? state.config.customRules.tiebreakPoints : 7;
    if (game[`player${winnerId}`] >= pointsToWin && game[`player${winnerId}`] - game[`player${loserId}`] >= 2) {
      gameWon = true;
    } else {
      const totalPoints = game.player1 + game.player2;
    ...
    // Normal game
    const wScore = game[`player${winnerId}`];
    const lScore = game[`player${loserId}`];
  ...
  if (gameWon) {
    state.sets[state.currentSet][`player${winnerId}`] += 1;
    ...
    if (state.config.format.startsWith('TIEBREAKS_ONLY')) {
      ...
      if (setFormatToUse === SET_FORMATS.SHORT_SET_4_TB_4_4) {
        gamesToWin = 4; tbScore = 4;
      } else if (setFormatToUse === SET_FORMATS.SHORT_SET_4_TB_5_5) {
        gamesToWin = 4; tbScore = 5;
      } else if (setFormatToUse === SET_FORMATS.FAST_4_TB_3_3) {
        gamesToWin = 4; tbScore = 3;
      } else if (setFormatToUse === SET_FORMATS.PRO_SET_8_TB_8_8) {
        gamesToWin = 8; tbScore = 8;
      } else if (setFormatToUse === SET_FORMATS.PRO_SET_10_TB_10_10) {
        gamesToWin = 10; tbScore = 10;
      } else if (setFormatToUse === SET_FORMATS.CUSTOM && state.config.customRules) {
        gamesToWin = state.config.customRules.gamesToWinSet;
        tbScore = state.config.customRules.tiebreakAt;
      }

      if (currentSet[`player${winnerId}`] >= Math.min(gamesToWin, tbScore)) {
        const needsMargin = setFormatToUse === SET_FORMATS.CUSTOM && state.config.customRules 
          ? state.config.customRules.winSetByTwo 
          : true;

        if (currentSet[`player${winnerId}`] >= gamesToWin && (!needsMargin || currentSet[`player${winnerId}`] - currentSet[`player${loserId}`] >= 2)) {
          setWon = true;
        } else if (currentSet[`player${winnerId}`] === tbScore + 1 && tbScore < gamesToWin) {
          setWon = true; // Won tiebreak
        } else if (currentSet[`player${winnerId}`] === tbScore && currentSet[`player${loserId}`] === tbScore) {
          if (tbScore > 0) { // If tbScore is 0, they play play-out? No, just no tiebreak.
    ...
    if (setWon) {
      ...
      if (format === MATCH_FORMATS.ONE_SET && setsWon[`player${winnerId}`] === 1) matchWon = true;
      if (format === MATCH_FORMATS.BEST_OF_3 || format === MATCH_FORMATS.TIEBREAKS_ONLY_BEST_OF_3) {
        if (setsWon[`player${winnerId}`] === 2) matchWon = true;
      }
      if (format === MATCH_FORMATS.BEST_OF_5 || format === MATCH_FORMATS.TIEBREAKS_ONLY_BEST_OF_5) {
        if (setsWon[`player${winnerId}`] === 3) matchWon = true;
      }
      if (format === MATCH_FORMATS.CUSTOM && state.config.customRules) {
        if (setsWon[`player${winnerId}`] === state.config.customRules.setsToWin) matchWon = true;
      }
      ...
      if (matchWon) {
        ...
        if (format.startsWith('TIEBREAKS_ONLY')) {
          state.currentGame.isTiebreak = true;
        } else if (format === MATCH_FORMATS.BEST_OF_3 && state.currentSet === 2 && state.config.setFormat === SET_FORMATS.SUPER_TIEBREAK_3RD_SET) {
          state.currentGame.isSuperTiebreak = true;
        } else if (format === MATCH_FORMATS.CUSTOM && state.config.customRules) {
          const finalSetIndex = (state.config.customRules.setsToWin * 2) - 2; // For best of 3 (setsToWin=2), final is set index 2.
          if (state.config.customRules.finalSetSuperTiebreak && state.currentSet === finalSetIndex) {
            state.currentGame.isSuperTiebreak = true;
          }
        }
      }
...
export const registerFault = (currentState) => {
  ...
  if (state.firstServeFault) {
    ...
    state.history.pop();
    const opponentId = state.server === 1 ? 2 : 1;
    return addPoint(currentState, opponentId, 'DOUBLE_FAULT');
  } else {
...
export const undoLastAction = (currentState) => {
  if (!currentState.history || currentState.history.length === 0) return currentState;
  const state = cloneState(currentState);
